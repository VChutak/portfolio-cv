...
**Per-project one-off components.** A couple of case studies need bespoke sections that don't generalize to other projects — these live in their own subfolder under `src/components/` (`src/components/bazinga/`, `src/components/nemesysco/`, e.g. `ColorSwatchGrid.astro`, `EmotionTimelineList.astro`) and are only ever imported by that one project's page. Follow this convention (a same-named subfolder) if a new project needs similar one-off sections, rather than adding more props/variants to the shared components in `src/components/`.

`src/layouts/InnerLayout.astro` is a dead/unused wrapper experiment (not imported anywhere, and it has a malformed `<slot>`). `src/pages/projects/LayoutProject.astro` is imported by `semrush.astro` and `stufstorage.astro` but never actually rendered in either — it's a leftover unused import, not live wrapping. Don't build on either without checking they're actually rendered first.
